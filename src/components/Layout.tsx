  ...
  return (
    <div className="flex h-screen bg-gray-100 dark:bg-gray-900">
      ...
      <div className={`${isSidebarMinimized ? 'w-16' : 'w-64'} bg-white dark:bg-gray-800 shadow-lg transition-all duration-300 ease-in-out`}>
            ...
            {navigation.map((item) => {
              ...
              return (
                <li key={item.name}>
                  <Link
                    to={item.href}
                    className={`flex items-center ${isSidebarMinimized ? 'justify-center' : 'space-x-3'} px-3 py-2 rounded-lg transition-colors ${
                      isActive
        ...
        <div className={`absolute ${isSidebarMinimized ? 'bottom-4 left-2' : 'bottom-16 left-4'} space-y-2`}>
          <button
            onClick={toggleTheme}
            className={`flex items-center ${isSidebarMinimized ? 'justify-center' : 'space-x-3'} px-3 py-2 text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-600 rounded-lg transition-colors`}
            title={isSidebarMinimized ? (theme === 'light' ? 'Dark Mode' : 'Light Mode') : undefined}
          >
